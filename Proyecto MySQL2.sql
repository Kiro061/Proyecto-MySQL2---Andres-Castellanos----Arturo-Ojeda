...
CREATE FUNCTION fn_CalcularTotalVenta (cantidad INT, precio_unitario DECIMAL(10,2))
RETURNS DECIMAL(10,2)
DETERMINISTIC
BEGIN
	DECLARE TotalVenta DECIMAL(10,2);
	SET TotalVenta = cantidad * precio_unitario;
	RETURN TotalVenta;
END;
...
BEGIN
	DECLARE v_edad INT;
	SET v_edad = TIMESTAMPDIFF(YEAR, c_fecha_nacimiento, CURDATE());
	RETURN v_edad;
END;
...
BEGIN
	...
	DECLARE v_apellido_limpio VARCHAR(100);
	DECLARE v_resultado VARCHAR(205);
	
	SET v_nombre_limpio = TRIM(C_nombre);
	SET v_apellido_limpio = TRIM(C_apellido);
	SET v_resultado = CONCAT(LOWER(v_apellido_limpio), '  ',LOWER(v_nombre_limpio));
